...
import vnreal.algorithms.BFSCoordinatedPathSplitting;
...
public class BFSNRPSWizard extends AbstractButtonDialog implements
	...
	public BFSNRPSWizard(Scenario scenario) {
	...
	protected void doAction() {
		...
		int maxHop = (Integer) kSpinner.getValue();

		// finally generate the algorithm
		IAlgorithm BFSCoordinatedPathSplitting = null;
		...
		if (!yesIsDistanceButton.isSelected()
			...
			BFSCoordinatedPathSplitting = new BFSCoordinatedPathSplitting(ns,
					wCpu, wBw, distance, true, true, maxHop);
		} else if (yesIsDistanceButton.isSelected()
				&& noNodeOverloadButton.isSelected()) {
			BFSCoordinatedPathSplitting = new BFSCoordinatedPathSplitting(ns,
					wCpu, wBw, distance, true, false, maxHop);
		} else if (noIsDistanceButton.isSelected()
				&& yesNodeOverloadButton.isSelected()) {
			BFSCoordinatedPathSplitting = new BFSCoordinatedPathSplitting(ns,
					wCpu, wBw, 0, false, true, maxHop);
		} else if (noIsDistanceButton.isSelected()
				&& noNodeOverloadButton.isSelected()) {
			BFSCoordinatedPathSplitting = new BFSCoordinatedPathSplitting(ns,
					wCpu, wBw, 0, false, false, maxHop);
		}
		new MyProgressBarDialog(BFSCoordinatedPathSplitting, scenario);
