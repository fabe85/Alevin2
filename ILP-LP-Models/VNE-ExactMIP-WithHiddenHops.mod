      ...
      /* objective function */
      minimize min: sum{(i,j) in SNtotal}sum{(k,l) in SNlinks} ((w1/(LinkCapacity[k,l]+w3))*flow[i,j,k,l]) + sum{i in SNnodes}(w2/(SNodeCapacity[i]+w3))*((sum{k in VNnodes}x[k,i]*VNodeDemand[k]) + (sum{(k,l) in SNtotal}sum{(i,j) in SNlinks}(if(k<>i) then flow[k,l,i,j]*HHFactor else 0)));
      ...
      s.t. CapacityLinkDem{(i,j) in SNlinks}: sum{(k,l) in SNtotal} if(i<>j) then flow[k,l,i,j] <= LinkCapacity[i,j];
