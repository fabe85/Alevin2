      ...
      /* objective function */
      minimize z: w1*(sum{(i,j) in SNlinks}sum{(k,l) in VNlinks} lambda[k,l,i,j]*VLinkDemand[k,l]) + w2*(sum{h in SNnodes}sum{(k,l) in VNlinks}sum{(h,j) in SNlinks}(if(k<>h and h<>l) then lambda[k,l,h,j]*HHDemand[k,l] else 0));
