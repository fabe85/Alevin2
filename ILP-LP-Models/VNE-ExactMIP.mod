      ...
      /* objective function */
      minimize min: w1*(sum{(k,l) in SNlinks}sum{(i,j) in SNtotal} flow[i,j,k,l]) + w2*(sum{i in SNnodes}sum{k in VNnodes}x[k,i]*VNodeDemand[k]);
      ...
      s.t. SourceII{(i,j) in SNtotal}: sum{(k,i) in SNlinks} flow[i,j,k,i] = 0;
      s.t. DestI{(i,j) in SNtotal}: sum{(k,j) in SNlinks} flow[i,j,k,j] = SLinkDemand[i,j];
      s.t. DestII{(i,j) in SNtotal}: sum{(j,l) in SNlinks} flow[i,j,j,l] = 0;
      ...
      s.t. CapacityLinkDem{(i,j) in SNlinks}: sum{(k,l) in SNtotal} flow[k,l,i,j] <= LinkCapacity[i,j];
      s.t. CapacityNodeDemI{i in SNnodes}: sum{k in VNnodes}x[k,i]*VNodeDemand[k] <= SNodeCapacity[i];
